...
def initialize_output_folder(video_path):
    ...
    video_path_wo_file = video_path.rsplit('/')[:-1]
    video_path_wo_file = '/'.join([str(obj) for obj in video_path_wo_file])

    output_folder_screenshot_path = f"{video_path_wo_file}/{video_path.rsplit('/')[-1].split('.')[0]}"
...
def convert_screenshots_to_pdf(output_folder_screenshot_path):
    output_slides_path = output_folder_screenshot_path.rsplit('/')[:-1]
    output_slides_path = '/'.join([str(obj) for obj in output_slides_path])
    output_pdf_path = f"{output_slides_path}/{video_path.rsplit('/')[-1].split('.')[0]}" + '.pdf'
...
if __name__ == "__main__":
    ...
    output_folder_screenshot_path = initialize_output_folder(video_path)
    print('screenshot path', output_folder_screenshot_path)
